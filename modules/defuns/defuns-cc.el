...
(defun doom|init-c/c++-settings ()
  (when (memq major-mode '(c-mode c++-mode objc-mode))
    ...
    ;; (define-key c++-mode-map ">" nil)
    (map! :map (c-mode-base-map c++-mode-map) ">" 'doom/autoclose->-maybe)
    (define-key c++-mode-map "<" nil)))
...
;;;###autoload
(defun doom/c-or-c++-header ()
  "Sets either c-mode or c++-mode, whichever is appropriate for the header."
  (let ((c-file (concat (f-no-ext (buffer-file-name)) ".c"))
        (cpp-file (concat (f-no-ext (buffer-file-name)) ".cpp")))
    (cond
        ((f-exists? c-file) (c-mode))
        ((f-exists? cpp-file) (c++-mode))
        ;; By default, set to C++
        (t (c++-mode)))))
